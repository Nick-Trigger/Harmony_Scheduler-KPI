...
class ClientARequest(BaseModel):
    ...
    changeover_matrix: _ChangeoverMatrixIn = Field(
        ..., alias="changeover_matrix_minutes"
    )
...
def parse_request(payload: dict[str, Any]) -> SchedulingProblem:
    ...
    products = tuple(
        Product(
            ...
            route=tuple(
                Operation(
                    capability=op.capability, duration_minutes=op.duration_minutes
                )
                for op in p.route
            ),
        )
    ...
    def _parse_changeover_key(key: str) -> tuple[str, str]:
        ...
        if len(parts) != 2:
            raise ValueError(
                f"Invalid changeover key: {key}, expected format 'from_family->to_family'"
            )
        return parts[0], parts[1]

    changeover_matrix = ChangeoverMatrix(
        values={
            _parse_changeover_key(k): v
            for k, v in request.changeover_matrix.values.items()
        }
    )
